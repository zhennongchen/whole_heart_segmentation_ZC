...
class Adapter3D(nn.Module):
    def __init__(self, dim, adapter_dim=64, act_layer=nn.GELU):
        super().__init__()

        self.norm = nn.LayerNorm(dim)
        self.down = nn.Linear(dim, adapter_dim, bias=True)
        ...
        self.act = act_layer()
        self.up = nn.Linear(adapter_dim, dim, bias=True)
    ...
    def forward(self, x, T, H, W):
        ...
        # 1) norm
        y = self.norm(x)   # [(B*T), H, W, C]

        # 2) down projection: C -> d
        y = self.down(y)   # [(B*T), H, W, d]

        # 3) reshape to 3D volume: [B, d, T, H, W]
        y = rearrange(y, '(b t) h w d -> b d t h w', b=B, t=T)

        # 4) 3D conv along temporal axis
        y = self.conv3d(y)

        # 5) back to [(B*T), H, W, d]
        y = rearrange(y, 'b d t h w -> (b t) h w d')

        # 6) activation + up projection
        y = self.act(y)
        y = self.up(y)     # [(B*T), H, W, C]
