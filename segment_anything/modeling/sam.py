...
from .custom_MaskDecoder import MaskDecoder
from .custom_PromptEncoder import PromptEncoder
...
class Sam(nn.Module):
    ...
    def __init__(
        ...
        if args.input_type == "2D":
          ...
          self.image_encoder = ImageEncoderViT_medivista(args, chunk=self.chunk, img_size = args.img_size, embed_dim = encoder_embed_dim,  depth = encoder_depth,  num_heads = encoder_num_heads, global_attn_indexes= encoder_global_attn_indexes ,multi = False, window_size = 14)
        ...
        # text prompt:
        self.text_prompt = [True if args.text_prompt == True else False][0]; print('Important! text prompt:', self.text_prompt)
        # box prompt
        self.box_prompt = args.box_prompt; print('Important! box prompt:', self.box_prompt)
    ...
    def forward_train(self, batched_input,  image_size, multimask_output = False):
        ...
        if self.input_type == "2DT":
          ...
          if self.text_prompt:
            text_prompt_feature = batched_input["text_prompt_feature"].cuda()
          else:
          ...
          sparse_embeddings, dense_embeddings = self.prompt_encoder(
              points=None, boxes=box_prompt_feature, masks=None,
              text = text_prompt_feature,
          )
